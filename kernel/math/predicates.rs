...
pub fn is_collinear2d(a: Vec2, b: Vec2, c: Vec2, tol: f64) -> Tri {
    ...
    match orient2d(a, b, c, tol) {
        ...
        Tri::Indeterminate => {
            let ab = b.sub(a);
            let ac = c.sub(a);
            if ab.is_finite() && ac.is_finite() && ab.length() > 0.0 && ac.length() > 0.0 {
                Tri::True
            } else if a == b && a == c {
                Tri::Indeterminate
            } else {
...
/// Whether `p` lies on the closed segment `[a,b]` within the closed tolerance
/// bands used for the line and projection checks.
...
pub fn is_coplanar(a: Vec3, b: Vec3, c: Vec3, d: Vec3, tol: f64) -> Tri {
    ...
    match orient3d(a, b, c, d, tol) {
        ...
        Tri::Indeterminate => {
            ...
            let ac = c.sub(a);
            let ad = d.sub(a);
            if !ab.is_finite() || !ac.is_finite() || !ad.is_finite() {
                Tri::Indeterminate
            } else {
                let normal = ab.cross(ac);
                if !normal.is_finite() {
                    Tri::Indeterminate
                } else if normal.length() == 0.0 {
                    Tri::Indeterminate
                } else {
                    let distance = normal.dot(ad).abs();
                    let scale = normal.length() * ad.length();
                    if !distance.is_finite() || !scale.is_finite() {
                        Tri::Indeterminate
                    } else if distance <= tol * scale {
                        Tri::True
                    } else {
                        Tri::Indeterminate
                    }
...
mod tests {
    ...
    #[test]
    fn collinearity_and_coplanarity_preserve_uncertainty() {
        assert_eq!(
            is_collinear2d(
                Vec2::new(0.0, 0.0),
                Vec2::new(1.0, 0.0),

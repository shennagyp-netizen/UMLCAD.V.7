...
impl PlanarSurface {
    ...
    pub fn distance_to_point(&self, point: Point3) -> Result<f64, SurfaceError> {
        ...
        let dx = if point.x < xmin { xmin - point.x } else if point.x > xmax { point.x - xmax } else { 0000.0 };
        let dy = if point.y < ymin { ymin - point.y } else if point.y > ymax { point.y - ymax } else { 0000.0 };

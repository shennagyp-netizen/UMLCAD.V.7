...
M11 is active from `main` commit `59b8f7849bd5d094b31088631006505e2eda61b6`. Current branch head `5d8fc8bfc3cd423eb887109da4f03061db1412a8` contains adaptive surface tessellation, certified line/arc trim-boundary sampling, convex trimmed-surface interior filling, conservative bounding spheres, deterministic parameter-space bounds, 8-way AABB subdivision, deterministic octree query structure with per-resident AABB filtering, and BVH node-pair candidate traversal with brute-force equivalence coverage. The tessellation source was reconstructed from the intact 781 baseline after CI exposed the earlier malformed write; the convex trim certificate permits collinear adaptive edge samples.

CI on `1330433d28587b844b0b15ff168467620a962dbd` exposed one Rust regression in `convex_trimmed_surface_fills_and_preserves_boundary`: adaptive collinear boundary samples caused a boundary-vertex fan to generate zero-area triangles whose centroids classified as `OnBoundary`. Diagnostic commit `d66723c4e2277da2f50463239fc628c5de9ef37f` identified the first rejected centroid at `u=1/512,v=0`. Commit `5d8fc8bfc3cd423eb887109da4f03061db1412a8` replaces that fan with a deterministic arithmetic-mean interior seed, explicitly certifies the seed as `Inside`, evaluates it once, and fans to each preserved boundary edge. This preserves fail-closed trim classification and does not broaden acceptance semantics.

...
mod tests {
    ...
    fn classify_with(a: &[&[f64]], b: &[f64], tol: f64) -> LinearConsistencyEvidence {
        let rows = a.len();
        let cols = a[0].len();
        let matrix = DMatrix::from_fn(rows, cols, |r, c| a[r][c]);
        let rhs = DVector::from_column_slice(b);
        classify_linear_system(&matrix, &rhs, tol, 1.0e10).unwrap()
    }

    fn classify(a: &[&[f64]], b: &[f64]) -> LinearConsistencyEvidence {
        classify_with(a, b, 1.0e-10)
    }
    ...
    #[test]
    fn nonfinite_positive_and_negative_infinity_are_rejected() {
        for value in [f64::INFINITY, f64::NEG_INFINITY] {
            let a = DMatrix::from_row_slice(1, 1, &[value]);
            let b = DVector::from_column_slice(&[1.0]);
            assert_eq!(
                classify_linear_system(&a, &b, 1.0e-10, 1.0e10),
                Err(LinAlgError::NonFinite)
        ...
        for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            let b = DVector::from_column_slice(&[value]);
            assert_eq!(
                classify_linear_system(&a, &b, 1.0e-10, 1.0e10),
                Err(LinAlgError::NonFinite)
            );
    ...
    #[test]
    fn rhs_dimension_mismatch_is_rejected() {
    ...
    fn invalid_tolerance_is_rejected() {
        ...
        for tol in [f64::NAN, f64::INFINITY, -1.0] {
            assert_eq!(
                classify_linear_system(&a, &b, tol, 1.0e10),
                Err(LinAlgError::InvalidTolerance)
            );
        }
        for threshold in [f64::NAN, f64::INFINITY, -1.0] {
            assert_eq!(
                classify_linear_system(&a, &b, 1.0e-10, threshold),
                Err(LinAlgError::InvalidTolerance)
            );
        }
    }
    ...
    #[test]
    fn row_scaling_preserves_consistency_and_rank_for_exactly_dependent_equations() {
        let base = classify(&[&[1.0, 1.0], &[2.0, 2.0]], &[2.0, 4.0]);
        let scaled = classify(&[&[1.0e-12, 1.0e-12], &[2.0e12, 2.0e12]], &[2.0e-12, 4.0e12]);
        assert_eq!(base.status, scaled.status);
        assert_eq!(base.coefficient_rank, scaled.coefficient_rank);
        assert_eq!(base.augmented_rank, scaled.augmented_rank);
    }
    ...
    #[test]
    fn equation_permutation_preserves_consistency() {
        let base = classify(&[&[1.0, 1.0], &[2.0, 2.0]], &[2.0, 4.0]);
        let permuted = classify(&[&[2.0, 2.0], &[1.0, 1.0]], &[4.0, 2.0]);
        assert_eq!(base.status, permuted.status);
        assert_eq!(base.coefficient_rank, permuted.coefficient_rank);
        assert_eq!(base.augmented_rank, permuted.augmented_rank);
    }

    #[test]
    fn redundant_consistent_equation_does_not_change_status() {
    ...
    #[test]
    fn dependent_rhs_perturbation_changes_consistent_system_to_inconsistent() {
        let evidence = classify(
            &[&[1.0, 2.0], &[0.0, 1.0], &[1.0, 3.0]],
            &[3.0, 2.0, 5.0 + 1.0e-4],
        );
    ...
    #[test]
    fn near_threshold_rhs_perturbation_is_controlled_by_declared_tolerance() {
        let a = [&[1.0, 1.0][..], &[2.0, 2.0][..]];
        let within = classify_with(&a, &[2.0, 4.0 + 0.5e-10], 1.0e-10);
        let outside = classify_with(&a, &[2.0, 4.0 + 1.0e-3], 1.0e-10);
        assert_eq!(within.status, LinearSystemStatus::UnderdeterminedConsistent);
        assert_eq!(outside.status, LinearSystemStatus::Inconsistent);
    }

    #[test]
    fn uniform_scaling_preserves_inconsistency() {
        let base = classify(&[&[1.0, 1.0], &[2.0, 2.0]], &[2.0, 5.0]);
        let scaled = classify(&[&[1.0e9, 1.0e9], &[2.0e9, 2.0e9]], &[2.0e9, 5.0e9]);
        assert_eq!(base.status, scaled.status);
        assert_eq!(base.coefficient_rank, scaled.coefficient_rank);
        assert_eq!(base.augmented_rank, scaled.augmented_rank);
    }

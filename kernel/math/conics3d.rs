...
pub struct Circle3 { pub center: Vec3, pub normal: Vec3, pub radius: f64 }
...
impl Circle3 {
    ...
    pub fn implicit_radial_value(&self,p:Vec3)->Result<f64,Conic3DError>{if!p.is_finite(){return Err(Conic3DError::NonFinite);}let n=self.unit_normal()?;let q=p.sub(self.center);let axial=q.dot(n);let radial=q.sub(n.scale(axial)).length();let v=radial-self.radius;if v.is_finite(){Ok(v)}else{Err(Conic3DError::NonFinite)}}
}
...
impl Sphere3 {
    ...
    pub fn implicit_value(&self,p:Vec3)->Result<f64,Conic3DError>{if!p.is_finite(){return Err(Conic3DError::NonFinite);}self.validate()?;let d=p.sub(self.center).length();let v=d-self.radius;if v.is_finite(){Ok(v)}else{Err(Conic3DError::NonFinite)}}
...
#[cfg(test)]
mod tests{use super::*;#[test]fn circle_parameterization_is_on_plane_and_radius(){let c=Circle3{center:Vec3::new(1.,2.,3.),normal:Vec3::new(0.,0.,2.),radius:4.};let p=c.point_at(0.125).unwrap();assert!((p.sub(c.center).length()-4.).abs()<1e-12);assert!((p.z-3.).abs()<1e-12);}#[test]fn sphere_equation_is_zero_on_known_point(){let s=Sphere3{center:Vec3::new(1.,2.,3.),radius:4.};assert!(s.squared_implicit(Vec3::new(5.,2.,3.)).unwrap().abs()<1e-12);}#[test]fn invalid_radius_is_rejected(){assert_eq!(Sphere3{center:Vec3::new(0.,0.,0.),radius:0.}.validate(),Err(Conic3DError::InvalidRadius));}}

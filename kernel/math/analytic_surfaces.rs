...
#[cfg(test)]
mod tests{use super::*;#[test]fn plane_is_affine_with_constant_derivatives(){let s=Plane3{origin:Vec3::new(1.,2.,3.),normal:Vec3::new(0.,0.,4.)};let j=s.evaluate(2.,5.).unwrap();assert!(j.du.cross(j.dv).length()>0.999);assert!(j.duu.length()<1e-15&&j.duv.length()<1e-15&&j.dvv.length()<1e-15);}#[test]fn sphere_has_radius_normal_and_pole_singularity(){let s=Sphere3{center:Vec3::new(0.,0.,0.),radius:2.};let j=s.evaluate(0.3,0.2).unwrap();assert!((j.point.length()-2.).abs()<1e-12);assert!((s.normal_at(0.3,0.2).unwrap().length()-1.).abs()<1e-14);assert_eq!(s.normal_at(0.3,std::f64::consts::FRAC_PI_2),Err(SurfaceEvalError::Singular));}#[test]fn cylinder_and_torus_have_orthogonal_parameter_derivatives(){let c=Cylinder3{origin:Vec3::new(0.,0.,0.),axis:Vec3::new(0.,0.,1.),radius:2.};let j=c.evaluate(0.7,3.).unwrap();assert!(j.du.dot(j.dv).abs()<1e-12);let t=Torus3{center:Vec3::new(0.,0.,0.),axis:Vec3::new(0.,0.,1.),major_radius:5.,minor_radius:1.};let j=t.evaluate(0.7,0.4).unwrap();assert!(j.point.is_finite()&&j.du.is_finite());}#[test]fn cone_apex_is_singular(){let c=Cone3{apex:Vec3::new(0.,0.,0.),axis:Vec3::new(0.,0.,1.),half_angle:0.5};assert_eq!(c.evaluate_positive(0.2,0.),Err(SurfaceEvalError::Singular));}}

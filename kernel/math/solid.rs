...
impl Triangle3{
 ...
 pub fn area(&self)->Result<f64,SolidError>{self.validate()?;let a=.5*self.b.sub(self.a).cross(self.c.sub(self.a)).length();if a.is_finite(){Ok(a)}else{Err(SolidError::Overflow)}}
 ...
 pub fn centroid_with_origin(&self)->Vec3{self.a.add(self.b).add(self.c).scale(.25)}
}
...
#[cfg(test)]
mod tests{use super::*;fn tetra()->PolyhedralSolid{let a=Vec3::new(0.,0.,0.);let b=Vec3::new(1.,0.,0.);let c=Vec3::new(0.,1.,0.);let d=Vec3::new(0.,0.,1.);PolyhedralSolid{triangles:vec![Triangle3{a:b,b:c,c:d},Triangle3{a:a,b:d,c:c},Triangle3{a:a,b:b,c:d},Triangle3{a:a,b:c,c:b}]}}#[test]fn tetrahedron_has_expected_volume_area_and_centroid(){let s=tetra();assert!((s.volume(1e-12).unwrap()-1./6.).abs()<1e-12);assert!((s.centroid(1e-12).unwrap().x-.25).abs()<1e-12);assert!((s.centroid(1e-12).unwrap().y-.25).abs()<1e-12);assert!((s.centroid(1e-12).unwrap().z-.25).abs()<1e-12);assert!((s.surface_area().unwrap()-(3.+3f64.sqrt())/2.).abs()<1e-12);}#[test]fn tetra_point_classification(){let s=tetra();assert_eq!(s.classify_point(Vec3::new(.1,.1,.1),Vec3::new(1.,.2,.3),1e-12).unwrap(),PointSolidClass::Inside);assert_eq!(s.classify_point(Vec3::new(2.,2.,2.),Vec3::new(1.,.2,.3),1e-12).unwrap(),PointSolidClass::Outside);}#[test]fn open_shell_is_rejected(){let mut s=tetra();s.triangles.pop();assert_eq!(s.validate_closed_oriented(1e-12),Err(SolidError::OpenShell));}}

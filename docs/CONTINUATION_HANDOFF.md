...
M11 is active from `main` commit `59b8f7849bd5d094b31088631006505e2eda61b6`. Current branch head `791cc625772f55718ae252e9146a00986ecb4f55` contains adaptive surface tessellation, certified line/arc trim-boundary sampling, convex trimmed-surface interior filling, conservative bounding spheres, deterministic parameter-space bounds, 8-way AABB subdivision, deterministic octree query structure with per-resident AABB filtering, and BVH node-pair candidate traversal with brute-force equivalence coverage. The tessellation source was reconstructed from the intact 781 baseline after CI exposed the earlier malformed write; the convex trim certificate permits collinear adaptive edge samples.

The first trimmed-surface defect was diagnosed as a zero-area fan caused by preserved collinear boundary samples. Commit `5d8fc8bfc3cd423eb887109da4f03061db1412a8` replaced the boundary-vertex fan with a deterministic arithmetic-mean interior seed, explicitly classified as `Inside`. That exposed a `MaxDepth` condition because centroid-only three-way refinement shrank edge spans too slowly for the declared 1e-3 angular target.

Commit `7dc02c73dab978c4abf228f3d75fc398d5f699b5` introduced midpoint sampling/refinement, and commit `abd9681d813dd88897086303696bb0b3e7d98b76` corrected its chord metric to the centroid-vs-triangle-average error plus three edge-midpoint-vs-edge-chord errors. Commit `791cc625772f55718ae252e9146a00986ecb4f55` changes the recursive split from four-way subdivision to longest-edge bisection, reusing the already classified midpoint and reducing branch growth from 4^depth to 2^depth while retaining the same fail-closed sampled error certificate.

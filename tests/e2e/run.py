#!/usr/bin/env python3
"""Single-command UMLCAD V6 integration/red-team gate.
...
The runner owns the real kernel-host lifecycle and executes:
  ...
  3. Existing .NET Rust-kernel E2E tests.
  4. Dedicated .NET black-box HTTP integration tests.
  5. Demo end-to-end build/evaluate test.
  6. Adversarial raw HTTP protocol probes.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="UMLCAD V6 one-command integration/red-team gate")
    ...
    try:
        ...
        passed &= runner.run(
            "dotnet-existing-kernel-e2e",
            [
                "dotnet",
                "test",
                "dotnet/tests/UMLCAD.Framework.Tests/UMLCAD.Framework.Tests.csproj",
                "--filter",
                "Category=KernelEndToEnd",
            ],
            900,
        )
        passed &= runner.run(
            ...
            [
                "dotnet",
                "test",
                "dotnet/tests/UMLCAD.Kernel.Integration.Tests/UMLCAD.Kernel.Integration.Tests.csproj",
            ],
            900,
        )
        passed &= runner.run(

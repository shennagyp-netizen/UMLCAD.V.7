...
#[cfg(test)]
mod tests{use super::*;fn box3(x:f64,y:f64,z:f64)->Aabb3{Aabb3::new(Vec3::new(x,y,z),Vec3::new(x+1.,y+1.,z+1.)).unwrap()}#[test]fn query_contains_all_bruteforce_overlaps(){let items=vec![(0,box3(0.,0.,0.)),(1,box3(.5,0.,0.)),(2,box3(3.,3.,3.)),(3,box3(3.5,3.,3.))];let b=Bvh3::build(&items).unwrap();let hits=b.query_aabb(box3(.25,.25,.25),1e-12);assert_eq!(hits,vec![0,1]);}#[test]fn deterministic_build_has_stable_candidate_pairs(){let items=vec![(3,box3(0.,0.,0.)),(1,box3(.5,0.,0.)),(2,box3(4.,0.,0.)),(0,box3(4.5,0.,0.))];let a=Bvh3::build(&items).unwrap().candidate_pairs(1e-12);let b=Bvh3::build(&items).unwrap().candidate_pairs(1e-12);assert_eq!(a,b);assert_eq!(a,vec![(1,3),(0,2)]);}#[test]fn invalid_box_is_rejected(){assert_eq!(Aabb3::new(Vec3::new(1.,0.,0.),Vec3::new(0.,1.,1.)),Err(BvhError::InvalidBounds));}}

...
impl Quaternion {
    ...
    pub fn from_axis_angle(axis:Vec3,angle:f64)->Result<Self,QuaternionError>{if!axis.is_finite()||!angle.is_finite(){return Err(QuaternionError::NonFinite);}let unit=axis.normalized().map_err(|_|QuaternionError::Degenerate)?;let half=.5*angle;let s=half.sin();Ok(Self::new(half.cos(),unit.x*s,unit.y*s,unit.z*s))}

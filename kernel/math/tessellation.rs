...
pub fn tessellate_curve3<F,G>(domain:(f64,f64),policy:TessellationPolicy,eval:F,tangent:G)->Result<TessellatedCurve3,TessellationError>where F:Fn(f64)->Result<Vec3,TessellationError>,G:Fn(f64)->Result<Vec3,TessellationError>{policy.validate()?;let(a,b)=domain;if!a.is_finite()||!b.is_finite()||b<=a{return Err(TessellationError::InvalidDomain);}let pa=eval(a)?;let pb=eval(b)?;if!pa.is_finite()||!pb.is_finite(){return Err(TessellationError::NonFinite);}let mut points=vec![pa];let mut parameters=vec![a];fn recurse<F,G>(a:f64,b:f64,pa:Vec3,pb:Vec3,depth:u32,p:&TessellationPolicy,eval:&F,tangent:&G,points:&mut Vec<Vec3>,params:&mut Vec<f64>)->Result<(),TessellationError>where F:Fn(f64)->Result<Vec3,TessellationError>,G:Fn(f64)->Result<Vec3,TessellationError>{let m=a+(b-a)*0.5;let pm=eval(m)?;let chord=point_segment_distance(pm,pa,pb)?;let ta=tangent(a)?;let tm=tangent(m)?;let tb=tangent(b)?;let angle=angle_between(ta,tm)?.max(angle_between(tm,tb)?);if chord<=p.chord_error&&angle<=p.angular_error{points.push(pb);params.push(b);return Ok(());}if depth>=p.max_depth{return Err(TessellationError::MaxDepth);}recurse(a,m,pa,pm,depth+1,p,eval,tangent,points,params)?;points.pop();params.pop();recurse(m,b,pm,pb,depth+1,p,eval,tangent,points,params)}recurse(a,b,pa,pb,0,&policy,&eval,&tangent,&mut points,&mut parameters)?;Ok(TessellatedCurve3{points,parameters,chord_error:policy.chord_error,angular_error:policy.angular_error,max_depth:policy.max_depth})}

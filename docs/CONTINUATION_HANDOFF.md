...
M11 is active from `main` commit `59b8f7849bd5d094b31088631006505e2eda61b6`. Head `6db875fb1be64fcd4bda469b22ce4cf8c14d06f2` contains adaptive surface tessellation, certified line/arc trim-boundary sampling, convex trimmed-surface interior filling, conservative bounding spheres, deterministic parameter-space bounds, 8-way AABB subdivision, deterministic octree query structure with per-resident AABB filtering, and BVH node-pair candidate traversal with brute-force equivalence coverage; its trimmed-surface regression now has sufficient declared depth for the existing normal-angle target. It is not yet M11-complete and is awaiting exact-head CI validation.
